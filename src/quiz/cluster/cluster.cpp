							 ...
							 std::unordered_set<int> &processed_indices)
{
	...
	std::vector<int> nearby_indices = tree->search(points[index], distanceTol);
	...
	for (int &idx : nearby_indices)
	{
		...
		{
			proximity(idx, points, cluster, tree, distanceTol, processed_indices);
		}
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol)
{
	...
	for (size_t i = 0; i < points.size(); i++)
	{
		...
		{
			std::vector<int> cluster;
			proximity(i, points, cluster, tree, distanceTol, processed_indices);
